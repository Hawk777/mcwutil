...
namespace {
	NBT::Tag tag_for_child_of_named_or_list(const Glib::ustring &name, const char *message) {
		...
		if (name == u8"long") return NBT::TAG_LONG;
		if (name == u8"float") return NBT::TAG_FLOAT;
		if (name == u8"double") return NBT::TAG_DOUBLE;
		if (name == u8"barray") return NBT::TAG_BYTE_ARRAY;
	...
	void check_list_subtype(NBT::Tag subtype) {
		switch (subtype) {
			...
			case NBT::TAG_LONG: return;
			case NBT::TAG_FLOAT: return;
			case NBT::TAG_DOUBLE: return;
			case NBT::TAG_BYTE_ARRAY: return;
	...
	void write_nbt(const FileDescriptor &nbt_fd, const xmlpp::Element *elt) {
		if (elt->get_name() == u8"named") {
			...
			const xmlpp::Attribute *name_attr = elt->get_attribute(u8"name");
			if (!name_attr) {
			...
			const xmlpp::Attribute *value_attr = elt->get_attribute(u8"value");
			if (!value_attr) {
			...
			const xmlpp::Attribute *subtype_attr = elt->get_attribute(u8"subtype");
			if (!subtype_attr) {
